...
2. qsokuはコマンドの文字列を組み立て（`//`の置き換えは[qsokufile_ja.md](qsokufile_ja.md#qsokufileの置いてある場所)のとおり）、カレントディレクトリのまま`sh`で次のように実行する：
   ...
   - `pwd`の行は`;`ではなく**改行**でつなぐ——コマンド末尾の`# …`コメントがこの行まで飲み込んでしまうのを避けるため。
...
| `QSOKU_CWD_FILE` | シェル連携の関数（[シェル連携](#シェル連携)参照）が一時ファイルのパスに設定する。設定されているとき、上のコマンドに`pwd`の行が足される。設定されていないとき（たとえばシェル関数を介さず直接qsokuを実行したとき）も`pwd`の行は足されるが、それを読むものが無いだけで害はない |
...
管理用コマンド（`.init`・`.add`など）も、`qsoku`の後に打てる名前として補完の対象になる。
